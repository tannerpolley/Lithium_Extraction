...
def solve_extents(oa, logK_li, logK_mg):
    V_aq = 1.0
    V_org = oa
    n_il0 = C_IL * V_org
    n_top0 = C_TOP * V_org
    ...
    def residual(y):
        ...
        n_hoe_aq = C_HOEMIM_AQ0 * V_aq + xi_li + xi_mg
...
def objective(p):
    ...
    for oa, (eli_t, emg_t) in target.items():
        eli, emg = solve_extents(oa, logK_li, logK_mg)
        errs.append((eli - eli_t) / 3.0)
        errs.append((emg - emg_t) / 0.3)
    return np.array(errs)

sol = least_squares(objective, x0=np.array([1.0, -2.0]), bounds=([-4.0, -10.0], [6.0, 2.0]), ftol=1e-12, xtol=1e-12, gtol=1e-12, max_nfev=200)
print('sol', sol.x)
for oa in sorted(target):
    eli, emg = solve_extents(oa, sol.x[0], sol.x[1])
