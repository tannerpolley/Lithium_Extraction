...
3. scenario-specific parameter CSVs under `data/reference/epcsaft_parameters/`
...
|---|---|
| water, Li+, Na+, K+, Cl-, Br- | Covered or expected covered in the local runtime path for Li/Na-focused aqueous support. |
| Mg2+, Ca2+, Sr2+, Ba2+, SO4/SO42- | Pretreatment/feed-context species for the active case, not extraction-equilibrium targets. Runtime gaps do not block the current Li/Na showcase as long as pretreatment is explicit. |
| HBTA | Gap. |
| TOPO | Gap. |
...
| Li-ligand complex | Gap for predictive ePC-SAFT parameters; current workflow uses a fitted reactive-stage HBTA/TOPO bridge. |
| Divalent-ligand complexes | Out of active scope because divalent ions are assumed pretreated away before the Li/Na extraction showcase. |
