...
def _evaluate_one(case_name: str, variant: dict[str, Any], timeout_seconds: float) -> dict[str, Any]:
    started = time.perf_counter()
    try:
        payload = _case_payload(case_name)
        mixture = ePCSAFTMixture.from_params(payload["params"], species=list(payload["species"]))
        option_payload = dict(variant["options"])
        if variant["operation"] == "lle":
            option_payload.setdefault("timeout_seconds", max(0.5, 0.50 * float(timeout_seconds)))
        options = EquilibriumOptions(**option_payload)
...
def _worker(out: mp.Queue, case_name: str, variant: dict[str, Any], timeout_seconds: float) -> None:
    out.put(_evaluate_one(case_name, variant, timeout_seconds))


def _run_one(case_name: str, variant: dict[str, Any], timeout_seconds: float) -> dict[str, Any]:
    if variant["operation"] == "lle":
        result = _evaluate_one(case_name, variant, timeout_seconds)
        result["timed_out"] = False
        result["wall_seconds_outer"] = result.get("wall_seconds")
        result["exitcode"] = 0
        return result
    q: mp.Queue = mp.Queue()
    proc = mp.Process(target=_worker, args=(q, case_name, variant, timeout_seconds))
    started = time.perf_counter()
    proc.start()
